...
static void enqueue_task_grr(struct rq *rq, struct task_struct *p, int flags) {
    ...
    raw_spin_lock(&grr_rq->runtime_lock);

    if (flags & ENQUEUE_HEAD) {
        list_add(&grr_se->run_list, &grr_rq->task_list);
    } else {
        list_add_tail(&grr_se->run_list, &grr_rq->task_list);
    }

    grr_rq->nr_running++;
    raw_spin_unlock(&grr_rq->runtime_lock);
...
static bool dequeue_task_grr(struct rq *rq, struct task_struct *p, int flags) {
    ...
    if (!list_empty(&grr_se->run_list)) {
        list_del_init(&grr_se->run_list);  /* Remove the task from runqueue */
        grr_rq->nr_running--;
        printk(KERN_INFO "GRR: Dequeued PID %d. Remaining tasks: %d\n", p->pid, grr_rq->nr_running);
        raw_spin_unlock(&grr_rq->runtime_lock);
        return true;  // Successfully dequeued the task
    }

    raw_spin_unlock(&grr_rq->runtime_lock);
    return false;  // Task was not in the runqueue
}
...
/* Helper: Retrieve the task_struct containing the provided grr_list entry */
...
/* Pick the next task to run from the GRR runqueue */
static struct task_struct *pick_next_task_grr(struct rq *rq, struct task_struct *prev) {
    struct grr_rq *grr_rq = &rq->grr;
    struct sched_grr_entity *grr_se;
    struct task_struct *next = NULL;

    /* Lock the GRR runqueue */
    raw_spin_lock(&grr_rq->runtime_lock);

    /* Select the first task in the GRR runqueue */
    if (!list_empty(&grr_rq->task_list)) {
        grr_se = list_first_entry(&grr_rq->task_list, struct sched_grr_entity, run_list);
        next = container_of(grr_se, struct task_struct, grr);
        list_move_tail(&grr_se->run_list, &grr_rq->task_list);  /* Round-robin scheduling */
    }
    ...
    if (next) {
        printk(KERN_INFO "GRR: Picked next task PID %d\n", next->pid);
    } else {
        printk(KERN_INFO "GRR: No runnable task. Returning idle.\n");
        next = rq->idle;  /* Fall back to idle task if no tasks are available */
    }
